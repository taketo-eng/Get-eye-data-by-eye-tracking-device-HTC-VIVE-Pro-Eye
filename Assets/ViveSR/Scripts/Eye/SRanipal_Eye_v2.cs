    ...
    namespace anipal
    {
        namespace Eye
        {
            public static class SRanipal_Eye_v2
            {
                ...
                public static bool GetPupilDiametre(EyeIndex eye, out float diametre, EyeData_v2 eye_data)
                {
                    bool valid = false;
                    //SRanipal_Eye_Frameworkが動作しているか確認
                    if (SRanipal_Eye_Framework.Status == SRanipal_Eye_Framework.FrameworkStatus.WORKING)
                    {
                        SingleEyeData eyeData = eye == EyeIndex.LEFT ? eye_data.verbose_data.left : eye_data.verbose_data.right;
                        valid = eyeData.GetValidity(SingleEyeDataValidity.SINGLE_EYE_DATA_PUPIL_DIAMETER_VALIDITY);
                        diametre = valid ? diametre = eyeData.pupil_diameter_mm : 0;
                        Debug.Log(diametre);
                    }
                    else
                    {
                ...
                public static bool GetPupilDiametre(EyeIndex eye, out float diametre)
                {
                    UpdateData();
                    return GetPupilDiametre(eye, out diametre, EyeData_);
                }
